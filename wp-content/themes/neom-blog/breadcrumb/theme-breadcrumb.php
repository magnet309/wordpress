<?php
function neom_breadcrumbs() {

	$showOnHome          = esc_html__( '1', 'neom-blog' );   // 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter           = '';   // Delimiter between breadcrumb.
	$home                = esc_html__( 'Home', 'neom-blog' );    // Text for the 'Home' link.
	$showCurrent         = esc_html__( '1', 'neom-blog' ); // Current post/page title in breadcrumb in use 1, Use 0 for don't show.
	...
	if ( is_home() || is_front_page() ) {
		...
		if ( is_category() ) {
			...
			echo $before . esc_html__( 'Archive by category', 'neom-blog' ) . ' "' . esc_html( single_cat_title( '', false ) ) . '"' . $after;

		} elseif ( is_search() ) {
			echo $before . esc_html__( 'Search results for ', 'neom-blog' ) . ' "' . esc_html( get_search_query() ) . '"' . $after;
		} elseif ( is_day() ) {
			...
			if ( get_post_type() != 'post' ) {
				...
				if ( $showCurrent == 1 ) {
					echo ' ' . esc_attr( $delimiter ) . '&nbsp' . wp_kses_post( $breadcrumb_seprator ) . '&nbsp' . $before . wp_kses_post( get_the_title() ) . $after;
				}
			} else {
			...
			if ( $showCurrent == 1 ) {
				echo ' ' . esc_attr( $delimiter ) . ' ' . $before . esc_html( get_the_title() ) . $after;
			}
			...
			for ( $i = 0; $i < count( $breadcrumbs ); $i++ ) {
				...
				if ( $i != count( $breadcrumbs ) - 1 ) {
					echo ' ' . esc_attr( $delimiter ) . '&nbsp' . wp_kses_post( $breadcrumb_seprator ) . '&nbsp';
				}
			}
			if ( $showCurrent == 1 ) {
				echo ' ' . esc_attr( $delimiter ) . ' ' . $before . esc_html( get_the_title() ) . $after;
			...
			echo $before . esc_html__( 'Posts tagged ', 'neom-blog' ) . ' "' . esc_html( single_tag_title( '', false ) ) . '"' . $after;
		} elseif ( is_author() ) {
			global $author;
			$userdata = get_userdata( $author );
			echo $before . esc_html__( 'Articles posted by ', 'neom-blog' ) . '' . $userdata->display_name . $after;
		} elseif ( is_404() ) {
			echo $before . esc_html__( 'Error 404 ', 'neom-blog' ) . $after;
		}
		...
		if ( get_query_var( 'paged' ) ) {
			...
			echo ' ( ' . esc_html__( 'Page', 'neom-blog' ) . '' . esc_html( get_query_var( 'paged' ) ) . ' )';
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
				echo '';
			}
		}

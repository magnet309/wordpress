...
class neom_Customizer_Notify_Section extends WP_Customize_Section {

	public $type = 'neom-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'neom_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'neom_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		foreach ( $neom_customizer_notify_recommended_actions as $key => $neom_lite_customizer_notify_recommended_action ) {
			if ( $neom_customizer_notify_show_recommended_actions[ $neom_lite_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $neom_lite_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$neom_lite_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $neom_lite_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $neom_customizer_notify_recommended_action['plugin_slug'] );
				$neom_lite_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $neom_lite_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$neom_lite_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$neom_lite_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$neom_lite_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$neom_lite_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$neom_lite_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$neom_lite_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$neom_lite_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$neom_lite_customizer_notify_recommended_action['button_label'] = $neom_deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $neom_lite_customizer_notify_recommended_action;
		}// End foreach().
		...
		foreach ( $neom_customizer_notify_recommended_plugins as $slug => $neom_plugin_opt ) {

			if ( ! $neom_plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $neom_lite_customizer_notify_show_recommended_plugins[ $slug ] ) && $neom_lite_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
			...
			switch ( $active['needs'] ) {
				...
				case 'deactivate':
					...
					$ti_customizer_notify_recommended_plugin['button_label'] = $neom_deactivate_button_label;
					break;
			}
			$neom_info                                     = $this->call_plugin_api( $slug );
			$ti_customizer_notify_recommended_plugin['id'] = $slug;
			$ti_customizer_notify_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $neom_plugin_opt['description'] ) ) {
				$ti_customizer_notify_recommended_plugin['description'] = $neom_plugin_opt['description'];
			} else {
				$ti_customizer_notify_recommended_plugin['description'] = $neom_info->short_description;
			}

			$ti_customizer_notify_recommended_plugin['title'] = $neom_info->name;
		...
		$json['total_actions']       = count( $neom_customizer_notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="neom-customizer-plugin-notify-actions-count">
							<span class="current-index">{{ data.recommended_actions[0].index }}</span>
							{{ data.total_actions }}
						</span>
					<# } #>
				</h3>
				<div class="neom-theme-recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_actions.length > 0 ){ #>
						<# for (action in data.recommended_actions) { #>
							<div class="neom-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
								<# if( !data.recommended_actions[action].check ){ #>
									<div class="neom-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_actions[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no neom-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
					...
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="neom-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="neom-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_plugins[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no neom-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
